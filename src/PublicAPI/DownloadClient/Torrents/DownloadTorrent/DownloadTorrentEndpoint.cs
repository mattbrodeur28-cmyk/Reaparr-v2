...
public class DownloadTorrentEndpointRequestValidator : Validator<DownloadTorrentEndpointRequest>
{
    public DownloadTorrentEndpointRequestValidator()
    {
        ...
        RuleFor(x => x.AudioQuality).IsInEnum();
        RuleFor(x => x.Type)
...
public class DownloadTorrentEndpoint : Endpoint<DownloadTorrentEndpointRequest>
{
    ...
    public override void Configure()
    {
        ...
        AllowAnonymous();
        PreProcessor<TorrentFileAuthenticationPreProcessor<DownloadTorrentEndpointRequest>>();
    }
    ...
    private async Task<MediaFileInfo?> GetFileInfoAsync(DownloadTorrentEndpointRequest req, CancellationToken ct)
    {
        ...
        if (req.Type == PlexMediaType.Song)
        {
            return await _dbContext
                .PlexMusicTrackData.Where(p => p.Id == req.PartId && p.PlexApiPartId == req.PlexApiPartId)
                .Select(p => new MediaFileInfo { FileName = p.GetFileName, Size = p.Size })
                .SingleOrDefaultAsync(ct);
        }
